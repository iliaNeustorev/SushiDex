<template>
    ...
    <div class="error-page">
        <header class="error-header">
            <Link :href="GeneralRoutes.index().url" class="error-brand" title="SushiDex — на главную">
                <span class="error-brand-mark">よ</span><span>SushiDex</span>
            </Link>
            <Link :href="GeneralRoutes.menu().url" class="error-menu-link">Наше меню <span aria-hidden="true">↗</span>
            </Link>
        </header>
...
.error-card {
    ...
    min-height: 540px;
    background: #fffaf4;
    box-shadow: 0 24px 70px rgba(51, 38, 27, .07);
}
...
.error-orbit {
    ...
    border: 1px solid rgba(240, 145, 114, .23);
    border-radius: 50%;
}
...
.error-page a:focus-visible, .error-page button:focus-visible {
